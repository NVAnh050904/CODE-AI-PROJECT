...
==========================
Level 5 - Person Retrieval (Part B): Attribute & Image Query Tool.

Queries reports/tracking/person_database.json supporting:
1. Attribute query (multi-label heads vs single-label top1).
2. Re-ID sample image query (OSNet 512-dim embedding cosine similarity).
3. Combined attribute filtering + Re-ID ranking.
4. Deduplicated identity display by identity_group_id.
5. Terminal table display & Result image grid generation.
6. Edge-case handling (0 results, missing search conditions, invalid query image).
7. Mandatory disclaimer notices.
"""
...
def rank_by_reid_similarity(records: list, query_image_path: str, extractor: ReIDExtractor) -> list:
    """
    Extract query image embedding and rank records by cosine similarity.
    Returns list of dicts with added field 'similarity_score'.
    """
    img_p = Path(query_image_path)
    if not img_p.exists():
        print(f"\n[LOI] Tep anh query-image khong ton tai tai: '{query_image_path}'")
        sys.exit(1)

    print(f"[INFO] Extracting Re-ID embedding for query image: '{img_p}'...")
    try:
        query_feats = extractor.extract_features([str(img_p)])  # (1, 512)
    except Exception as e:
        print(f"\n[LOI] Khong the doc hoac giai ma tep anh query-image tai: '{query_image_path}' ({e})")
        sys.exit(1)

    if query_feats is None or len(query_feats) == 0:
        print(f"\n[LOI] Khong the trich xuat dac trung Re-ID cho tep anh: '{query_image_path}'")
        sys.exit(1)
...
def main():
    ...
    # Edge Case 2: No search conditions specified at all
    active_conds = get_active_conditions(args)
    if not active_conds:
        print("\n[LOI] Can it nhat 1 dieu kien tim kiem (thuoc tinh hoac --query-image). Vi du: --gender female --upper_color red")
        sys.exit(1)

    # Edge Case 3 (Early check): Check query_image existence before loading heavy database
    if args.query_image:
        img_p = Path(args.query_image)
        if not img_p.exists():
            print(f"\n[LOI] Tep anh query-image khong ton tai tai: '{args.query_image}'")
            sys.exit(1)

    db = load_database()
    print(f"[INFO] Loaded person database containing {len(db)} total track records.")

    # Step 1: Attribute Filtering
    filtered_recs = filter_by_attributes(db, args)
    print(f"[INFO] Attributes Filter Matched: {len(filtered_recs)} track records.")

    # Step 2: Re-ID Image Ranking (if requested)
    ...
    # Step 3: Deduplicate by identity_group_id
    final_results = deduplicate_by_identity_group(ranked_recs)

    # Slice top-k
    top_results = final_results[:args.top_k]

    # Edge Case 1: 0 results after filtering
    if not top_results:
        print("\n" + "=" * 80)
        print("[THONG BAO] Khong tim thay nguoi nao phu hop voi dieu kien tim kiem.")
        print("=" * 80)
        print("\nCac dieu kien tim kiem da su dung:")
        for k, v in active_conds.items():
            print(f"  - {k}: {v}")

        if args.glasses or args.bag:
            print("\n" + "!" * 80)
            print("Luu y: head 'glasses' va 'bag' co F1-score thap khi train (49.32% va 67.15%) do mau hiem trong dataset - co the co nguoi phu hop nhung bi bo sot.")
            print("!" * 80)

        # Early exit: DO NOT generate grid image, DO NOT print output grid path line
        return

    # Step 4: Terminal Output Table Display (results > 0)
    print("\n" + "=" * 105)
    ...
    if is_reid_query:
        ...
        header = f"{'No':<5} | {'Global / Group ID':<34} | {'Video Name':<26} | {'Attributes (G / Up / Low / Bag)':<35}"
    print(header)
    print("-" * 105)
    ...
    print("=" * 105)
    print(f"Total Unique Identities Returned: {len(top_results)} (out of {len(final_results)} matches)")

    # Step 5: Save Grid Image ONLY when top_results exist
    ts_str = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    grid_filename = f"result_{ts_str}.png"
    grid_path = generate_result_grid_image(top_results, grid_filename)
    if grid_path:
        print(f"\n[OUTPUT GRID IMAGE] Saved result grid image to: '{grid_path}'")

    # Step 6: MANDATORY WARNING MESSAGES
    if is_reid_query:
        print("\n" + "!" * 80)
        print("Luu y: do chinh xac Re-ID dao dong 5-22% tuy dieu kien camera (xem TECHNICAL_REPORT.md muc Level 3) - ket qua la tin hieu tham khao, khong phai ket luan chac chan.")
        print("!" * 80)
